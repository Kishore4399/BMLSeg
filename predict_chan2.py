...
def predict(args, model, device, dataDict, output_dir):
    ...
    with torch.no_grad():
        ...
        # cv2.imwrite(filename=os.path.join(output_dir, meta['filename'] + '_00_pred.bmp'), img=pred[0, ...] * 255)
        # cv2.imwrite(filename=os.path.join(output_dir, meta['filename'] + '_01_pred.bmp'), img=pred[1, ...] * 255)
