...
from lib.data.med_transforms import AddBackgroundChannel, HistogramEqualization, CV2Loader
...
def predict(args, model, device, dataDict, output_dir):
    ...
    with torch.no_grad():
        ...
        pred = torch.zeros(input_img.shape[0], 3, input_img.shape[2], input_img.shape[3]).to(device)
        count = torch.zeros(input_img.shape[0], 3, input_img.shape[2], input_img.shape[3]).to(device)
            ...
            for j in range(1 + math.ceil((image_size[1] - patch_size[1]) / stride[1])):
                ...
                if args.tta == 1:
                    ...
                    for n, (flip_dim, rotate_angle) in enumerate(fft_combine):
                        ...
                        temp_output = torch.unsqueeze(temp_output, dim=1)
        ...
        dice_bml = dice[:, 1, ...].mean()
        dice_bone = dice[:, 2, ...].mean()
        ...
        # cv2.imwrite(filename=os.path.join(output_dir, meta['filename'] + '_pred.bmp'), img=combined_pred * 255)
    ...
    for i, model_path in enumerate(model_paths):
        ...
        with tqdm(total=len(dataPath_list), desc="predicting.....") as pbar:
            for fdic in dataPath_list:
                ...
                fdic['label'] = os.path.join(args.data_path, fdic['label'])
        ...
        mean_dice_bone = np.nanmean(dice_bones)
        mean_HD95_bml = np.nanmean(HD95_bmls)
        mean_HD95_bone = np.nanmean(HD95_bones)

        mean_dice_bmls.append(mean_dice_bml)
        mean_HD95_bmls.append(mean_HD95_bml)
        mean_dice_bones.append(mean_dice_bone)
        mean_HD95_bones.append(mean_HD95_bone)
        print(
            f"{args.model_version}: model {i} | "
            f"Mean Dice bml: {mean_dice_bml:.04f} | "
            f"Mean Dice bone: {mean_dice_bone:.04f} | "
            f"Mean HD95 bml: {mean_HD95_bml:.04f} | "
            f"Mean HD95 bone: {mean_HD95_bone:.04f}")

    results = pd.DataFrame(data=np.stack([mean_dice_bmls, mean_dice_bones, mean_HD95_bmls, mean_HD95_bones], axis=1),
    columns=['Dice BML', 'Dice bone', 'HD95 BML', 'HD95 bone'])
